...
依赖: 同 lx_camera_rgbd_driver / sam3_exemplar_segment / mask_filter_pointcloud；
可选 3D 窗口: pip install open3d

用法（在 get_object 目录或设置 PYTHONPATH）:
  python3 rgbd_sam3_live.py --text \"cotton yarn bobbin\"
  python3 rgbd_sam3_live.py --text \"hand\" --instance 0 --no-o3d
...
from mrdvs.sam3_seg import Sam3Segmenter, parse_box
...
def main() -> int:
    ...
    p.add_argument("--device", default="", help="cpu 或 0")
    ...
    try:
        seg = Sam3Segmenter(weights=wpath, device=args.device, half=args.half, conf=args.conf)
    ...
    try:
        ...
        while True:
            ...
            line0 = f"fps~{fps_smooth:.1f} stride={args.stride}"
